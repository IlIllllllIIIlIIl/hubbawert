...
async function loadCategories() {
    ...
    json.categories.forEach(cat => {
        const buttonContent = (cat.image ? `<img src="_dat/serve/img/wert/furni/${cat.image}" width="16" height="16" loading="lazy">&nbsp;` : '') + cat.name;
        let editBtn = '';
        let hasEdit = '';
        
        if(isEditor) {
            hasEdit = 'has-edit';
            editBtn = '<button type="button" class="btn btn-dark btn-sm mb-2 edit-btn" title="Bearbeiten">✏️</button>';
        }
        
        let categoryData = [cat.id, buttonContent, hasEdit, editBtn];
        let categoryToAdd = categoryTemplate;
        for (let j = 0; j < categoryReplace.length; j++) {
            categoryToAdd = categoryToAdd.replace(categoryReplace[j], categoryData[j]);
        }
        container.insertAdjacentHTML('beforeend', categoryToAdd);
    });
    ...
    if(isEditor) {
        container.insertAdjacentHTML('beforeend', `
            <div class="col-md-6">
                <form class="d-flex gap-2">
                    <input type="text" class="form-control form-control-sm mb-2" placeholder="Neue Kategorie" required>
                    <button type="button" class="btn btn-dark btn-sm mb-2" onclick="addCategory(this)">➕</button>
                </form>
            </div>
        `);

        // Add event listeners for edit buttons
        document.querySelectorAll('#categories .edit-btn').forEach(btn => {
        ...
        document.querySelectorAll('#categories a[data-id]').forEach(link => {
            link.addEventListener('click', (e) => {
                e.preventDefault();
                window.location.href = `?c=${link.dataset.id}`;
            });
        });
    }
}
...
async function addCategory(btn) {
    ...
    if(response.ok) {
        input.value = '';
        loadCategories(); // Reload categories instead of page refresh
    } else {
...
async function editCategory(id, name) {
    ...
    if(response.ok) {
        ...
        const error = await response.json();
        alert(error.error || 'Fehler beim Bearbeiten der Kategorie');
    }
}

async function deleteCategory(id) {
    if(confirm('Möchtest du diese Kategorie löschen?')) {
        const formData = new FormData();
...
// Initialize
setTooltips();
document.querySelector('#categories').addEventListener('show.bs.modal', loadCategories);
