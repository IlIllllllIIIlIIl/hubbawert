...
$isAdmin = isset($u_details['id']) && $u_details['rank'] >= 7 ? 1 : 0;
if($isEditor){
...
if(empty($imageHash)){
    ...
    $update->execute([$_POST['itemName'], $_POST['itemDesc'], $price, $categories, $_POST['oldName']]);
}else{
    $update = $core->m->prepare('UPDATE furniture_rare_details SET item_name=?, longdesc=?, price=?, category=?, image=? WHERE item_name=?');
    $update->execute([$_POST['itemName'], $_POST['itemDesc'], $price, $categories, $imageHash, $_POST['oldName']]);
...
// Add category data as a dataset attribute
$itemTemplate = str_replace('id="{id}"', 'id="{id}" data-categories="'.$item['category'].'"', $itemTemplate);
...
<div class="cats">
<div class="row">';
// Generate categories HTML for both modal buttons and select options
...
while ($cat = $select->fetch(PDO::FETCH_ASSOC)) {
    // Generate HTML for modal buttons
    ...
    $categories_html .= '<option value="'.$cat['id'].'">'.htmlspecialchars($cat['name']).'</option>';
}
$pagecontent .= '</div>
</div>
</div>
...
const categoriesHtml = \''.$categories_html.'\';
let rarity = '.$rarity.';
