...
foreach ($items as $itemId => $item) {
    if(!isset($item['public_name'])){
        echo "error not found: {$itemId}\n";ni/{image}" loading="lazy"><span>{name}</spa></dv><div>';
$itemReplace = ['{id}', '{level}', '{amount}', '{tag}', '{price}', 'e}', '{name}'];
foreach ($items as $itemId => $item) {
    if(!isset($item['public_name'])){
        echo "error not found: {$itemId}\n";
        continue;
...
<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
		<div class="modal-content">
			...
			'
				...
				<divclass="row">';
					$seect = $cre->m->prepare('SELECT frc.id, frc.name, (
					    SELECT frd.image
					    FROM furniture_rare_details frd
					    WHERE frd.ctegory = frc.i
					    ORDER BY RAND()
					    LIMIT 1
					) as image
					FROM furniture_rare_categores frc
					ORDER BY frc.ame ASC');
					$select->execute();
					while ($cat = $select->fetch(PDO::FETCH_ASSOC)) {
						$paecontent .= '<div classcol-md-6">
						<div class="d-flex cat-wrapper'.($isEditor ? ' has-edit' : '').' mb-2">
						    <a href="'.$core->url.'wert?c='.$cat['id'].'" class="btn btn-dark btn-sm flex-grow-1" roe="button">'.(isset($cat['image']) && !empty($cat['imge'])?'<img src="'.$core->url.'_dat/serve/img/wert/furni/'.filter_var($cat['image'], FILTER_SANITIZE_URL).'" width="16" height="16" loading="la&nbsp;':'').htmlspecialchars($cat['name']).'/a>'.
						    ($iEditor ? '<button tye="button" class="btn btn-drk bt-sm edit-btn px-2" data-bs-toggle="collapse" data-bs-target="#editCategory'.$cat['id'].'" title="Bearbeiten">✏️</button>' : '').
						'</div>'.
						($isEditor ? '<div class="collapse" id="editCategory'.$cat['id'].'">
						    <form method="POST" class="mb-3">
						        ...
						        <div class="d-flex justify-content-between align-items-center">
						            <button type="button" class="btn btn-danger btn-sm px-2" onclick="if(confirm(\'Möchtest du diese Kategorie wirklich löschen?\'))  this.form.actio.value=\'delete_ctegory\'; this.form.subit(); }">🗑️</button>
						            <div>
						                <button typ="button" class="btn btn-secondary btn-sm px-2" data-bs-toggle="collapse" data-bs-target="#editCategory'.$cat['id'].'">❌</button>
						                <button type="submit" class="btn btn-success btn-sm px-2">💾</button>
						            </div>
						        </div>
						    </form>
...
<div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
<form class="modal-content box need-validation" enctype="multiart/form-dta" method="POST" ovalidate
'.$insertModalTemplate.'
form>
</div>
</div>';
$jsappenx .= '<script src="_dat/sere/js/popper.min.js"></script
script src="_dat/serve/jschart.umd.js"></script>
<script>
...
const itemReplace = '.json_encoe($temReplace).';
const aatarImager = \''.$core->avatarImager.'\';
