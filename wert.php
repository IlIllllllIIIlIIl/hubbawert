...
if ($isEditor){
	...
	$isEdit = isset($_POST['oldName']);
		...
		$allowedMime = ['image/png', 'image/jpeg', 'image/pjpeg', 'image/gif'];
		...
		<div class="d-flex">
			<div class="w-50" style="border:1px solid #2c2e3c; padding:12px">
				...
				<div style="display:flex; align-items:center; margin-bottom:10px">
					...
					<input class="form-control" style="width:150px; margin-left:auto" name="price" type="number" min="0" value="0" autocomplete="off" required>
				</div>
...
$itemTemplate = '
<div class="col-md-4">
	<div class="box item" id="{id}" data-categories="{categories}">
		<img class="rarity l{level}" title="{amount}">
		<span{tag}>{price}</span>
		<img src="_dat/serve/img/wert/furni/{image}" loading="lazy">
		<span>{name}</span>
	</div>
</div>';
		...
		<div class="modal-content">
			...
			<div class="modal-body">'.
				...
				'<div class="cats collapse category-toggle show">
					<div class="row">';
						$select = $core->m->prepare('SELECT cat.id, cat.name, (
							SELECT rare.image
							FROM furniture_rare_category_mappings map
							LEFT JOIN furniture_rare_details rare ON (rare.id = map.furniture_id)
							WHERE map.category_id = cat.id
							ORDER BY map.furniture_id LIMIT 1) as image
						FROM furniture_rare_categories cat
						...
						$select->execute();
							...
							while ($cat = $select->fetch(PDO::FETCH_ASSOC)) {
								$pagecontent .= '<div class="col-md-6">
									...
									'</div>'.
									($isEditor ? '
									<div class="collapse" id="editCategory'.$cat['id'].'">
										<form method="POST" class="mb-3">
